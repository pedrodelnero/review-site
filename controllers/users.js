...
export const updateUser = async (req, res) => {
  ...
  try {
    user.password = await user.updatePassword(currentPassword, newPassword, confirmNewPassword);
    const updatedUser = await User.findByIdAndUpdate(user._id, user, { new: true });

    !updatedUser ? res.status(404).json({ error: 'No user with ID provided' }) : res.status(200).json({ user: updatedUser, message: 'Product successfully updated.' })
  } catch (error) {
